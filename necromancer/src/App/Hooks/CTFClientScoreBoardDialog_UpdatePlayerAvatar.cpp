...
// Signatures for scoreboard color hooks only
...
// Also skips avatar updates for streamer mode hidden players (Amalgam pattern)
	...
	void* rcx, int playerIndex, void* kv)
{
	...
	// Amalgam: if (!(F::PlayerUtils.GetNameType(playerIndex) & NameTypeEnum::Privacy)) CALL_ORIGINAL(...)
