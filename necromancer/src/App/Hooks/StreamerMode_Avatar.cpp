...
// Exact copy from Amalgam - CAvatarImage::SetAvatarSteamID
MAKE_HOOK(CAvatarImage_SetAvatarSteamID, Signatures::CAvatarImage_SetAvatarSteamID.Get(), bool, __fastcall,
	void* rcx, CSteamID steamIDUser, EAvatarSize avatarSize)
{
	// Amalgam: if (!(F::PlayerUtils.GetNameType(steamIDUser.GetAccountID()) & NameTypeEnum::Privacy)) return CALL_ORIGINAL(...)
	if (!StreamerMode::ShouldHide(steamIDUser.GetAccountID()))
		return CALL_ORIGINAL(rcx, steamIDUser, avatarSize);

	return CALL_ORIGINAL(rcx, CSteamID(), avatarSize);
}

// Exact copy from Amalgam - CAvatarImagePanel::SetPlayer
MAKE_HOOK(CAvatarImagePanel_SetPlayer, Signatures::CAvatarImagePanel_SetPlayer.Get(), void, __fastcall,
	void* rcx, CSteamID steamIDForPlayer, EAvatarSize avatarSize)
{
	// Amalgam: if (!(F::PlayerUtils.GetNameType(steamIDForPlayer.GetAccountID()) & NameTypeEnum::Privacy)) return CALL_ORIGINAL(...)
	if (!StreamerMode::ShouldHide(steamIDForPlayer.GetAccountID()))
		return CALL_ORIGINAL(rcx, steamIDForPlayer, avatarSize);

	CALL_ORIGINAL(rcx, CSteamID(), avatarSize);
}

// Exact copy from Amalgam - CTFMatchSummary::UpdatePlayerAvatar
MAKE_HOOK(CTFMatchSummary_UpdatePlayerAvatar, Signatures::CTFMatchSummary_UpdatePlayerAvatar.Get(), void, __fastcall,
	void* rcx, int playerIndex, KeyValues* kv)
{
	if (!StreamerMode::ShouldHide(playerIndex))
		CALL_ORIGINAL(rcx, playerIndex, kv);
}

// Exact copy from Amalgam - CTFHudMannVsMachineScoreboard::UpdatePlayerAvatar
MAKE_HOOK(CTFHudMannVsMachineScoreboard_UpdatePlayerAvatar, Signatures::CTFHudMannVsMachineScoreboard_UpdatePlayerAvatar.Get(), void, __fastcall,
	void* rcx, int playerIndex, KeyValues* kv)
{
	if (!StreamerMode::ShouldHide(playerIndex))
		CALL_ORIGINAL(rcx, playerIndex, kv);
...
// Exact copy from Amalgam - CTFHudMatchStatus::UpdatePlayerAvatar
MAKE_HOOK(CTFHudMatchStatus_UpdatePlayerAvatar, Signatures::CTFHudMatchStatus_UpdatePlayerAvatar.Get(), void, __fastcall,
	void* rcx, int playerIndex, KeyValues* kv)
{
	if (!StreamerMode::ShouldHide(playerIndex))
		CALL_ORIGINAL(rcx, playerIndex, kv);
