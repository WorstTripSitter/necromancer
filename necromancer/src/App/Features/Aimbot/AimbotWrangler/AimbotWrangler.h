...
class CAimbotWrangler
{
	...
	// Pre-generated splash sphere points (generated once per radius)
	std::vector<std::pair<Vec3, int>> m_vecSplashSpherePoints = {};
	float m_flLastSplashRadius = 0.0f;
	...
	// Splash prediction for rockets - Amalgam style with point types
	std::vector<std::pair<Vec3, int>> GenerateSplashSphere(float flRadius, int nSamples);
	...
	// Simulate rocket path and check if it can reach the destination
	...
	// Draw visualization (rocket path, movement prediction, splash point)
	void DrawVisualization(C_ObjectSentrygun* pSentry, const WranglerTarget_t& target, bool bIsRocket);
